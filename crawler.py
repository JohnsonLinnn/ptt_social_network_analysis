...
class PttWebCrawler(object):
    ...
    def parse_articles(self, start, end, board, path='.', timeout=3):
            ...
            for i in range(end-start+1):
                ...
                resp = requests.get(
                    ...
                    url = self.PTT_URL + '/bbs/' + board + '/search?page='+str(index)+'&q=團購',
                    #search?page=28&q=recommend%3A30
                    #'/search?page='+str(index)+'&q=recommend%3A30'
    ...
    def parse(link, article_id, board, timeout=3):
        ...
        for push in pushes:
            ...
            if push_tag == u'推':
                p += 1
                pid.append( push_userid )
            elif push_tag == u'噓':
                b += 1
                bid.append(push_userid)
            else:
                n += 1
                nid.append(push_userid)
        pid=list(dict.fromkeys(pid))
        bid=list(dict.fromkeys(bid))
